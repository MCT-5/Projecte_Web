...
class Game(models.Model):
    ...
    # ── campos nuevos: enlaces externos ───────────────────────────────────
    igdb_id      = models.IntegerField(null=True, blank=True, db_index=True, unique=True)
    steam_app_id = models.IntegerField(null=True, blank=True, db_index=True)

    # ── campos nuevos: datos enriquecidos de IGDB ─────────────────────────
    description  = models.TextField(blank=True)
    rating       = models.FloatField(default=0)

    # ── campos nuevos: estadísticas de SteamSpy ───────────────────────────
    steamspy_owners     = models.BigIntegerField(default=0)
    steamspy_positive   = models.IntegerField(default=0)
    steamspy_negative   = models.IntegerField(default=0)
    steamspy_playtime   = models.IntegerField(default=0, help_text="Minutos de juego medio (histórico)")
    steamspy_players_2w = models.IntegerField(default=0, help_text="Jugadores activos últimas 2 semanas")
    steamspy_tags       = models.CharField(max_length=500, blank=True)
    ...
    @property
    def score_pct(self):
        """Porcentaje de reseñas positivas de Steam."""
        total = self.steamspy_positive + self.steamspy_negative
        if total == 0:
            return None
        return round(self.steamspy_positive / total * 100, 1)
